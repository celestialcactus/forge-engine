...
| Attributable learning foundation | `CLI8A-MEMORY-FOUNDATION` | Slices 0–2 accepted through PR #32 candidate `e9e8cd9` and Checkpoint 92; Slice 3 explicitly authorized 2026-08-31 | Implement only repo-scoped autosave `off|ask|auto`, Rust-validated standing grants, visible attribution, and purge-style undo. Preserve inactive retrieval; Slices 4–5 and CLI8B/C remain gated. |
...
4. ADR-0038 and ADR-0039 settle the memory boundary, and the
   ...
   authorized on 2026-08-31. Runtime retrieval remains gated by CLI8B; Slices 4–5
...
1. Implement and gate only the approved CLI8A Slice 3 proof: repo-scoped autosave
   `off|ask|auto`, Rust-validated standing grants, visible save attribution, and
   non-blocking purge-style undo. Preserve the accepted no-retrieval boundary.
