...
| --- | ---: | --- |
| Rust lifecycle and verified local-change machinery | 8 / 10 | Strong design and adversarial fixtures; cross-platform hosted evidence exists through 5A. It still lacks 5B hosted acceptance and power-loss coverage for the outer run. |
...
| Approval/control | Rust resolves versioned host-policy and user-consent facts. Cancellation/deadline paths are proven. 5B budgets are implemented locally but not yet hosted-accepted; policy profiles remain 5C. | Configurable sandbox and approval policies, granular approval categories, managed settings, and review flows. | Layered allow/ask/deny permissions, managed settings, hooks, and sandbox policy. | Tool/path/URL allow/deny, persisted permissions, hooks, admin restrictions, and autopilot continuation limits. | Dangerous-command approval, file-write controls, platform authorization, and container policy. |
...
1. Finish 5B hosted Rust parity plus live Ollama/OpenAI and controlled VS Code regression.
2. Implement 5C as a small product policy profile over the existing fact contract; do not add another policy engine.
3. Add minimum outer-run recovery: append the canonical events/artifact, resume only idempotent work, and surface retained non-idempotent transactions without replaying them.
4. Package the native kernel with the TypeScript CLI for clean Windows and macOS installation; add `doctor`, effective configuration, and upgrade smoke tests.
5. Resolve the root open-source license with owner/legal review and add contribution/security guidance.
6. Ship an alpha test kit: five representative prompts, expected evidence, known limitations, issue template, and telemetry that is local/opt-in.
