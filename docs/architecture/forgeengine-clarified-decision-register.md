...
| 5 | Memory semantics | ADR-0038/0039; Slices 0–2 accepted through PR #32 / Checkpoint 92, Slice 3 accepted through PR #33 / Checkpoint 93, and Slice 4 authorized on 2026-09-02 | Rust owns deterministic identity, exact scope, provenance, the canonical NDJSON ledger, rebuild, correction, recovery, grants, and erasure rewrite; TypeScript owns orchestration, bounded capture-candidate construction, UX, and future replaceable retrieval machinery. `ask` is default; only a local user action may create a current-repository `auto` grant stored under the exact developer actor. Slice 4 may implement only forget/restore, explicit privacy purge, and recovery-history clear through the frozen atomic rewrite and retention boundary. Developer-profile grants, context injection, retrieval, skills, Slice 5, and CLI8B/C remain gated. |
