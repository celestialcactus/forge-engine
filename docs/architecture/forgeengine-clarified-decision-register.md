...
| 5 | Memory semantics | ADR-0038/0039; Slices 0–2 accepted through PR #32 / Checkpoint 92 and Slice 3 accepted for merge through PR #33 / Checkpoint 93 | Rust owns deterministic identity, exact scope, provenance, the canonical NDJSON ledger, rebuild, correction, recovery, grants, and erasure rewrite; TypeScript owns orchestration, bounded capture-candidate construction, UX, and future replaceable retrieval machinery. `ask` is default; only a local user action may create a current-repository `auto` grant stored under the exact developer actor. Slice 3 adds bounded autosave and narrow immediate undo plus corrected TTY/non-TTY interaction adapters without moving authority out of Rust. Developer-profile grants, forget/purge/history-clear, context injection, retrieval, skills, and Slices 4–5 remain gated. |
