...
| 5 | Memory semantics | ADR-0038/0039 and Slices 0–2 accepted through PR #32 / Checkpoint 92; Slice 3 candidate `afa6e67` locally gated | Rust owns deterministic identity, exact scope, provenance, the canonical NDJSON ledger, rebuild, correction, recovery, grants, and erasure rewrite; TypeScript owns orchestration, bounded capture-candidate construction, UX, and future replaceable retrieval machinery. `ask` is default; only a local user action may create a current-repository `auto` grant stored under the exact developer actor. Slice 3 implements autosave and narrow immediate undo only; hosted/merge acceptance remains. Developer-profile grants, forget/purge/history-clear, context injection, retrieval, and skills remain gated. |
