...
| 5 | Memory semantics | ADR-0038/0039 and Slices 0–2 accepted through PR #32 / Checkpoint 92; Slice 3 authorized | Rust owns deterministic identity, exact scope, provenance, the canonical NDJSON ledger, rebuild, correction, recovery, grants, and erasure rewrite; TypeScript owns orchestration, capture-candidate construction, UX, and future replaceable retrieval machinery. `ask` is default; only a local user action may create an actor/scope-bound `auto` grant. Slice 3 implements autosave and undo only. Forget/purge/history-clear, context injection, retrieval, and skills remain gated. |
