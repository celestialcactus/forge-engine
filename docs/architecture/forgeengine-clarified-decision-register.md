...
| 5 | Memory semantics | ADR-0038/0039; Slices 0–3 accepted through PRs #32–33 / Checkpoints 92–93; Slice 4 candidate `20b9bac` accepted for merge through Checkpoint 94 | Rust owns deterministic identity, exact scope, provenance, the canonical NDJSON ledger, rebuild, correction, recovery, grants, and erasure rewrite; TypeScript owns orchestration, bounded capture-candidate construction, UX, and future replaceable retrieval machinery. `ask` is default; only a local user action may create a current-repository `auto` grant stored under the exact developer actor. Slice 4 adds recoverable forget/restore, selected-lineage purge, and recovery-history clear through the frozen atomic rewrite and retention boundary; its output states that independent runs, artifacts, conversations, backups, and media are not erased. Developer-profile grants, context injection, retrieval, skills, Slice 5, and CLI8B/C remain gated. |
