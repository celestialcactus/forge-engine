...
| 5 | Memory semantics | ADR-0038/0039; Slices 0–4 accepted through PRs #32–34 / Checkpoints 92–94; Slice 5 preview is the active unaccepted candidate | Rust owns deterministic identity, exact scope, provenance, the canonical NDJSON ledger, rebuild, correction, recovery, grants, erasure rewrite, and final preview admission/reasons; TypeScript owns orchestration, bounded capture-candidate construction, and UX. `ask` is default; only a local user action may create a current-repository `auto` grant stored under the exact developer actor. Slice 4 adds recoverable forget/restore, selected-lineage purge, and recovery-history clear through the frozen atomic rewrite and retention boundary; its output states that independent runs, artifacts, conversations, backups, and media are not erased. Slice 5 may only preview baseline eligibility over the exact repository and developer scopes; Forge does not insert that memory into planner/provider prompt context. Developer-profile grants, retrieval, skills, and CLI8B/C remain gated. |
