...
impl CleanRevisionWorktreeAdapter {
    ...
    pub fn try_new_with_isolation_provider(
        ...
        let verification = VerificationRunner::try_new_with_isolation_provider(
            config.verification_checks.clone(),
            isolation_provider,
        )?;
...
impl ChangeTransactionAdapter for CleanRevisionWorktreeAdapter {
    ...
    fn verify(
        ...
        let evidence =
            self.verification
                .execute(&prepared.candidate_path, selection, cancellation)?;
        if !self.original_workspace_unchanged(&prepared)? {
